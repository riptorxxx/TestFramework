...
#__________________________________________________________________________
    ...
    # def select_cache_disks(self, cluster_disks: ClusterDisks,
    #                        count: int,
    #                        used_disks: set
    #                        ) -> tuple[List[str], Optional[int]]:
    #
    #     logger.debug(f"Starting select_cache_disks with parameters:")
    #     logger.debug(f"Count required: {count}")
    #     logger.debug(f"Used disks: {used_disks}")
    #     logger.debug(f"Available free_for_wc disks: {cluster_disks.free_for_wc}")
    #     logger.debug(f"All disks info: {cluster_disks.disks_info}")
    #
    #     available_disks = [
    #         disk for disk in cluster_disks.free_for_wc
    #         if disk not in used_disks and
    #            cluster_disks.disks_info[disk]['type'] == "SSD"
    #     ]
    #     logger.debug(f"Filtered available SSD disks: {available_disks}")
    #
    #     selected_disks = self._validate_and_return(available_disks, count, "cache disks")
    #     logger.debug(f"Selected disks after validation: {selected_disks}")
    #
    #     if selected_disks:
    #         disk_size = cluster_disks.disks_info[selected_disks[0]]['size']
    #         logger.debug(f"Selected disk size: {disk_size}")
    #         return selected_disks, disk_size
    #     return selected_disks, None




    # def select_spare_disks(self, cluster_disks: ClusterDisks,
    #                        count: int,
    #                        main_disk: str,
    #                        used_disks: set
    #                        ) -> tuple[List[str], Optional[int]]:
    #     main_disk_info = cluster_disks.disks_info[main_disk]
    #     available_disks = [
    #         disk for disk in cluster_disks.free_disks
    #         if disk not in used_disks and
    #            self._match_disk_parameters(cluster_disks.disks_info[disk], main_disk_info)
    #     ]
    #     selected_disks = self._validate_and_return(available_disks, count, "spare disks")
    #
    #     if selected_disks:
